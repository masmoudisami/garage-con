...
class User {
    ...
    /**
     * Vérifie s'il existe déjà des utilisateurs actifs dans le système
     * @return bool true si des utilisateurs existent, false sinon
     */
    ...
    /**
     * Vérifie s'il existe déjà des administrateurs actifs
     * @return bool true si des admins existent, false sinon
     */
    public function hasActiveAdmins() {
        $stmt = $this->db->query("SELECT COUNT(*) as count FROM users WHERE active = 1 AND role = 'admin'");
        $result = $stmt->fetch();
        return $result['count'] > 0;
    }
}