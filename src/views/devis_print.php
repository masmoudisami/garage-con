...
<head>
    <meta charset="UTF-8">
    <title>Devis N° <?php echo str_pad($devis['id'], 6, '0', STR_PAD_LEFT); ?></title>
    <style>
        @page {
            margin: 15mm 15mm;
            size: A4;
        ...
        * {
        ...
        .garage-name { font-size: 16px; font-weight: bold; color: #2c3e50; margin: 5px 0; }
        .garage-details { font-size: 11px; line-height: 1.5; color: #555; }
        .invoice-info { text-align: right; }
        .invoice-title { font-size: 20px; font-weight: bold; color: #2c3e50; margin-bottom: 10px; }
        .invoice-number { font-size: 12px; margin-bottom: 5px; }
        ...
        .totals p { margin: 5px 0; font-size: 11px; }
        ...
        @media screen {
            .invoice-container { border: 1px solid #ddd; padding: 30px; box-shadow: 0 0 10px rgba(0,0,0,0.1); }
            .no-print { display: block; margin-top: 20px; text-align: center; padding: 20px; background: #ecf0f1; }
        }
    </style>
</head>
<body>
    ...
    <div class="invoice-container">
        <div class="header">
            ...
            <div class="invoice-info">
                ...
                <div class="invoice-number">N° <?php echo str_pad($devis['id'], 6, '0', STR_PAD_LEFT); ?></div>
                <div class="invoice-number">Date: <?php echo date('d/m/Y', strtotime($devis['devis_date'])); ?></div>
                <?php if($devis['validity_date']): ?>
                <div class="invoice-number">Valable jusqu'au: <?php echo date('d/m/Y', strtotime($devis['validity_date'])); ?></div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="client-section">
            ...
            <?php if(!empty($client['matricule_fiscal'])): ?>Mat. Fiscal: <?php echo htmlspecialchars($client['matricule_fiscal']); ?><br><?php endif; ?>
            ...
            <?php if($devis['mileage'] > 0): ?>
            <div style="margin-top: 10px;"><strong>Kilométrage:</strong> <?php echo number_format($devis['mileage'], 0, ',', ' '); ?> KM</div>
            <?php endif; ?>
        </div>
        ...
        <div class="totals">
            <p>Total HT: <strong><?php echo number_format($devis['total_ht'], 3, ',', ' '); ?></strong></p>
            <p>TVA (<?php echo $devis['tax_rate']; ?>%): <strong><?php echo number_format($devis['total_tva'], 3, ',', ' '); ?></strong></p>
            <p>Droit de timbre: <strong><?php echo number_format($devis['droit_timbre'], 3, ',', ' '); ?></strong></p>
            <p class="total-ttc">Total TTC: <?php echo number_format($devis['total_ttc'], 3, ',', ' '); ?></p>
            
            <div class="amount-in-words">
                <span class="label">Arrêté le présent devis à la somme de :</span>
                <strong><?php echo convertNumberToWords($devis['total_ttc']); ?></strong>
            </div>
            
            <?php if($devis['validity_date']): ?>
            <div class="validity-info">
                ⏰ <strong>Validité :</strong> Ce devis est valable jusqu'au <?php echo date('d/m/Y', strtotime($devis['validity_date'])); ?>
            </div>
            <?php endif; ?>
        </div>

        <?php if(!empty($devis['comment'])): ?>
        <div style="margin-top: 20px; padding: 15px; background: #f9f9f9; border-radius: 5px;">
            <div class="client-title">COMMENTAIRE</div>
            <?php echo nl2br(htmlspecialchars($devis['comment'])); ?>
        </div>
        <?php endif; ?>

        <div class="footer">
            <?php echo GARAGE_NAME; ?> - <?php echo GARAGE_ADDRESS; ?> - Tél: <?php echo GARAGE_PHONE; ?><br>
            Devis établi le <?php echo date('d/m/Y'); ?> - Merci de votre confiance
        </div>
    </div>
    ...
    <div class="no-print">
        ...
        <a href="index.php?route=devis" class="btn" style="padding: 10px 20px; background: #e74c3c; color: #fff; text-decoration: none; border-radius: 4px; margin-left: 10px;">↩️ Retour</a>
